...
//hiactor中没有emplace_back、erase_tail、reset三个函数
//从banyan拷贝来的path_eos中用到前两个，先添加进来
...
struct column_batch : public fixed_column_batch<T> {
    // 以下两个函数为hiactor中原本的column_batch，未注释的部分均从banyan中拷贝过来
    ...
    static column_batch<T> load_from(hiactor::serializable_queue&& qu) {
        return column_batch<T>(std::move(qu.pop()[0]));
    }
